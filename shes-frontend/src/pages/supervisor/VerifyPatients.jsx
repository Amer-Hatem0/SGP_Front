...
export default function VerifyPatients() {
    ...
    const fetchPatients = async () => {
        try {
            const response = await axios.get(`${API_BASE_URL}/Supervisor/Patients`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            setPatients(response.data); // expected: [{ userId, fullName, email, phone }]
        } catch (err) {
            console.error('Error fetching patients:', err);
        }
    };

    const verifyPatient = async (userId) => {
        try {
            await axios.post(`${API_BASE_URL}/Supervisor/VerifyPatient/${userId}`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
        ...
        <>
            ...
            <div className="d-flex">
                <div className="flex-grow-1">
                    <div className="container mt-5">
                        <div className="card shadow border-0 p-4">
                            <h5 className="mb-4 fw-bold">Pending Patient Verifications</h5>
                            {patients.length === 0 ? (
                                <p className="text-muted text-center">No unverified patients found.</p>
                            ) : (
                                <table className="table table-bordered table-hover shadow-sm">
                                    <thead className="table-light">
                                        <tr>
                                        ...
                                        {patients.map((p, index) => (
                                            <tr key={p.userId}>
                                                ...
                                                <td>
                                                    <button
                                                        className="btn btn-success btn-sm"
                                                        onClick={() => verifyPatient(p.userId)}
                                                    >
                                                        Verify
                                                    </button>
                                                </td>
